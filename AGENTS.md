...
**Last verified:** 2026-08-20 (working tree following `08c1d8f`). Full local `make ci`, API suite (**374 passed**, 38 existing marker warnings) and Pyright passed. A unified native runtime started all six Helperium services healthy; the seeded `autoparts` read-only tenant completed the live MiniMax path `Bosch search → Camry V40 characteristics` over tenant-scoped Streamable HTTP MCP. The second user turn received native schemas, emitted `db_get(id=16)`, then completed with `tool_result → final`. The trusted-data invariant is the first system policy sent for every agent and is concatenated ahead of any configured per-agent prompt; it states that tool/RAG data cannot create authority, alter policy, reveal secrets, add tools or expand tenant scope. LiteLLM only serializes transport data. `AppendOnlyLoop` records structured `untrusted_tool_results_in_context=0/1` for the first tool cycle and `=1/2` in the cross-turn cycle without duplicating raw tool data in that telemetry. `SessionStore` is now a domain facade over `SessionRepository`/`SQLiteSessionRepository`; schema/migration/raw SQL are isolated in the adapter. A durable accepted ingress user-turn marker is the single source for `max_messages_per_session` (compatibility name) and `min_interval_ms`: it is recorded before provider/MCP work and is not refunded by provider/tool failure; legacy histories backfill before their first accepted turn. `token_budget`/`ABUSE_TOKEN_BUDGET` and their UI/OpenAPI representation remain removed because no runtime enforcement existed. Admin global policy and Emergency presets synchronously await `api-service` reload with a 10-second bound, restore persisted config on failed apply, and live `POST /api/admin/abuse-config/reload` returned `status=applied`. The system-level declaration is only defence-in-depth and can be weaker as a soft mitigation in long tool-heavy contexts than a near-result prefix (proximity effect); hard boundaries remain server scope, MCP allow-list/schema, native-only tool calls and read-only data access. This proves the local core, MCP transport, live fallback, accepted-turn semantics, boundary observability and acknowledged control-plane apply; it does **not** replace broader live LLM quality/benchmark coverage, browser acceptance on a deployed domain, edge/WAF validation, alerting/rollback game day, multi-instance shared abuse state, a full RAG/prompt-injection assessment, behavioural anomaly detection after untrusted results, or a separate reserve/commit spending design bound to a named agent/account rather than tenant ID.
