...
**Last verified:** 2026-08-20 (anti-abuse change committed after baseline `3f0f34e`). Full local `make ci` passed twice on the final working tree. A unified native runtime started all six Helperium services healthy; the seeded `autoparts` read-only tenant completed the live MiniMax path `Bosch search → Camry V40 characteristics` over tenant-scoped Streamable HTTP MCP. The second user turn received native schemas, emitted `db_get(id=16)`, then completed with `tool_result → final`; provider telemetry recorded `untrusted_tool_results=1` on the first continuation and `=2` on the cross-turn continuation. Tool result content is framed only in the provider-wire copy by `_TOOL_RESULT_BOUNDARY`; the canonical stored transcript stays unchanged. Session quota is durable SQLite state (`user_turn_count`, `last_user_turn_at`) updated only by user messages, with legacy backfill; assistant/tool messages do not consume it. `token_budget`/`ABUSE_TOKEN_BUDGET` and their UI/OpenAPI representation are removed because no runtime enforcement existed. Admin global policy and Emergency presets synchronously await `api-service` reload with a 10-second bound, restore persisted config on failed apply, and live `POST /api/admin/abuse-config/reload` returned `status=applied`. Full tests include the provider wire framing/non-mutation regressions, durable user-turn state regression, and dashboard rollback regression. This proves the local core, MCP transport, live fallback, anti-abuse boundary and acknowledged control-plane apply; it does **not** replace broader live LLM quality/benchmark coverage, browser acceptance on a deployed domain, edge/WAF validation, alerting/rollback game day, multi-instance shared abuse state, a full RAG/prompt-injection assessment, behavioural anomaly detection after untrusted results, or a separate reserve/commit spending design bound to a named agent/account rather than tenant ID.
