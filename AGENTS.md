...
2026-08-18 (working tree after `8725612`) — direct-chat tenant policy is now a dedicated `tenant_authority.py` resolver rather than inline route code. A real ScriptedLLM E2E proves persisted composite named-agent scope traverses api-service → MCPClient → gateway prefixed tool → data-service → SSE despite hostile request tenant header; rebuilt authenticated stack passed 128 deterministic E2E tests.
