...
**Last verified:** 2026-08-20 (worktree atop `1972a6d`). Full local `make ci` passed after clean LiteLLM model/provider routing, ordered provider fallback and current-turn continuation regressions. A unified native runtime started all six services healthy; the seeded `autoparts` read-only tenant completed tenant-scoped Streamable HTTP MCP sessions with public SSE `tool_call → tool_result → token → final → done`. A configured `polza` primary completion failed live and the generic fallback adapter switched the same turn to the persisted `ollama` MiniMax candidate. LiteLLM's published capability now suppresses schemas only for the immediate continuation after the **current** tool result; historical `role: tool` context from an earlier user turn never suppresses schemas for a new turn. Structured adapter telemetry records model/provider, schema count, current-turn continuation and capability result without logging prompts or results. The real MiniMax cross-turn path `Bosch search → Camry V40 characteristics` was verified after the fix: the second user request received schemas, emitted native `db_get(id=16)`, and completed without a raw tool transcript. The local public storefront Caddy was also verified with native upstream `host.docker.internal:8081`: `https://localhost/embed/embed.js` and the registered-agent widget config both returned `200`. This proves the deterministic local core, MCP transport, live fallback and cross-turn MiniMax tool delivery; it does **not** replace broader live LLM quality/benchmark coverage, browser acceptance on a deployed domain, edge/WAF validation, alerting/rollback game day or a full RAG/prompt-injection assessment.
