...
**Last verified:** 2026-08-20 (working tree atop `acf8056`). Full local `make ci` passed after clean LiteLLM model/provider routing and transcript wire-serialization regressions. A unified native runtime started all six services healthy; the seeded `autoparts` read-only tenant completed a tenant-scoped Streamable HTTP MCP session, `db_search` tool call/result and a final MiniMax answer with the public SSE sequence `tool_call → tool_result → token → final → done` using raw model ID plus an explicit LiteLLM provider slug. The continuation wire request is determined inside the LiteLLM adapter from its published function-calling capability, not per-agent API/config. The local public storefront Caddy was also verified with native upstream `host.docker.internal:8081`: `https://localhost/embed/embed.js` and the registered-agent widget config both returned `200`. This proves the deterministic local core, MCP transport, one live MiniMax tool turn and local public embed delivery; it does **not** replace broader live LLM quality/benchmark coverage, browser acceptance on a deployed domain, edge/WAF validation, alerting/rollback game day or a full RAG/prompt-injection assessment.
