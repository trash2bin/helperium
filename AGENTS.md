...
**Last verified:** 2026-08-20 (fallback restoration worktree atop `38b4169`). Full local `make ci` previously passed after clean LiteLLM model/provider routing and transcript wire-serialization regressions; the current full Python suite also passed after ordered execution-time fallback coverage was added. A unified native runtime started all six services healthy; the seeded `autoparts` read-only tenant completed a tenant-scoped Streamable HTTP MCP session, `db_search` tool call/result and a final MiniMax answer with the public SSE sequence `tool_call → tool_result → token → final → done`. A configured `polza` primary completion then failed live and the generic fallback adapter switched the same `autoparts` turn to the persisted `ollama` MiniMax candidate, which completed the MCP-backed turn successfully. The continuation wire request is determined inside the LiteLLM adapter from its published function-calling capability, not per-agent API/config. The local public storefront Caddy was also verified with native upstream `host.docker.internal:8081`: `https://localhost/embed/embed.js` and the registered-agent widget config both returned `200`. This proves the deterministic local core, MCP transport, live MiniMax tool and primary-to-fallback delivery; it does **not** replace broader live LLM quality/benchmark coverage, browser acceptance on a deployed domain, edge/WAF validation, alerting/rollback game day or a full RAG/prompt-injection assessment.
