...
2026-08-18 (working tree after `267974c`) — MCP production hardening complete: `MCP_REQUIRE_AUTH=true` fails fast without a key, Origin allow-list and bounded composite scope validation added, direct chat uses server-configured `DEFAULT_TENANT_ID`, named agents retain persisted composite authority, and the rebuilt authenticated stack passed 127 deterministic E2E tests including cross-scope session replay rejection.
2026-08-18 (working tree after `3749daa`) — direct-chat tenant policy is now a dedicated `tenant_authority.py` resolver rather than inline route code. A real ScriptedLLM E2E proves one persisted composite named-agent scope traverses api-service → MCPClient → two gateway-prefixed tools → two distinct data-service tenant DBs → SSE despite hostile request tenant header; rebuilt authenticated stack passed 128 deterministic E2E tests.
