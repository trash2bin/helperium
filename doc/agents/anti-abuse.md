...
Все `role: tool` results остаются raw canonical evidence в session/transcript storage. Текстовый system-prompt invariant для недоверенного контента сознательно не используется: поведение модели контролируется структурой, а не текстовыми уговорами (контракт "No prompt steering" в `AGENTS.md`). `system_prompt` агента задаётся только через admin config; дефолт — минимальная заглушка в `agent/prompts.py`.

Никакой text parser не вводится; исполняются только нормализованные native tool calls. Перед каждым completion `AppendOnlyLoop` структурно считает `role: tool` records в фактически отправляемом context и записывает `untrusted_tool_results_in_context` в structured loop log и LLM-call backlog metadata. Raw tool content в этой telemetry не дублируется.

Hard boundary находится не в промпте, а в gateway и данных: server-resolved tenant IDs попадают в `X-Tenant-ID`; MCP allow-list/schema и data-service read-only limits не могут быть расширены текстом tool result. Отдельный future control — behavioural anomaly-check для sensitive tool calls сразу после untrusted data — ещё не реализован и не должен считаться активной защитой.
