...
func TestGenerate_GrepForGrades(t *testing.T) {
	...
	// Проверяем, что консолидированные MCP tools генерируются (Фаза 2)
	...
	if !hasSearchTool {
		t.Error("expected db_search MCP tool (consolidated, Phase 2)")
	}
}
...
func TestGenerate_StrategyEndpoints(t *testing.T) {
	...
	// Проверяем, что консолидированный MCP tool db_search сгенерирован (Фаза 2)
	...
	if searchTool == nil {
		t.Fatal("expected db_search MCP tool")
	}
	if len(searchTool.Params) < 3 {
		t.Errorf("expected at least 3 params on db_search (entity+pattern+limit), got %d", len(searchTool.Params))
	}
}
...
func TestGenerate_BoolFilterParams(t *testing.T) {
	...
	// REST filter-endpoint существует (совместимость) и имеет Strategy "filter".
	if filterEp.Strategy != "filter" {
		t.Fatalf("expected filter strategy, got %q", filterEp.Strategy)
	}

	// Пер-энтити MCP tool filter_products существует (Фаза 2.5).
	var hasDBFilter bool
	for _, tool := range cfg.MCPTools {
	...
	if !hasDBFilter {
		t.Fatal("expected filter_products MCP tool (per-entity, Phase 2.5)")
	}

	// Типизация bool/исключение шумовых полей проверяем через FilterStrategy
	// (реальная логика рантайма — ToolParams строит per-field параметры):
	// is_active (business flag) → bool; is_promo (marketing) и system-поля → нет.
	var entityCfg config.Entity
	for _, e := range cfg.Entities {
		if e.Name == "products" {
			entityCfg = e
			break
	...
	filterStrategy := search.NewFilterStrategy("id", "name")
	params := filterStrategy.ToolParams(entityCfg)
	...
	// System datetime/date fields are excluded from filter params.
...
func TestGenerate_DisplayNameOnStrategyTools(t *testing.T) {
	...
	for _, tool := range cfg.MCPTools {
		...
		case strings.HasPrefix(tool.Name, "get_"):
			// get_* больше не эмитится (MCP-политика, Фаза 1) — если появится,
			// должен иметь DisplayName. Тут fallback для future-политики.
			if tool.DisplayName == "" {
