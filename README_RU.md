...
**2. Автоматическая генерация.** Система читает схему базы и сама создаёт инструменты для агента: `grep_товары` (текстовый поиск с учётом нескольких слов и regex), `filter_товары` (фильтрация по полям: цена, категория), `schema_товары` (разведочный анализ: уникальные значения, диапазоны), `get_товары` (по ID), `count_товары`, `distinct_товары`. Ничего писать не нужно.
