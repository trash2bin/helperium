...
**2. Автоматическая генерация.** Система читает схему базы и сама создаёт инструменты для агента. Итоговая поверхность — N пер-энтити `filter_{entity}` (например, `filter_товары` — фильтрация по полям: цена, категория; имена полей прямо в схеме инструмента) + 6 консолидированных `db_*` (`db_map` — карта БД, `db_describe` — разведка сущности, `db_search` — текстовый поиск с multi-token и regex, `db_filter` — фильтрация по значениям (fallback), `db_get` — по id из результата поиска, `db_related` — один хоп по FK). Полный перечень и fallback-поведение: [services/data-service/README.md#mcp-тулы](services/data-service/README.md#mcp-тулы). Ничего писать не нужно.
...
### Полная локальная демка с внешним магазином автозапчастей

Встроенный `demo/web` на `:8080` и Django-магазин
`demo/autoparts-store` на `:8000` — это разные приложения. Для ручной проверки
обоих контуров используй явный opt-in флаг:

```bash
./infra/scripts/dev.sh start --with-autoparts
open http://127.0.0.1:8080  # Helperium dev web
open http://127.0.0.1:8000  # внешний магазин автозапчастей
```

Обычный `./infra/scripts/dev.sh start` магазин не запускает, а
`./infra/scripts/dev.sh stop` намеренно его не останавливает. Флаг лишь запускает
собственный Compose-стек storefront; он не регистрирует магазин как tenant и не
встраивает виджет автоматически. Для живой интеграции отдельно выполни onboarding
tenant и агента.
