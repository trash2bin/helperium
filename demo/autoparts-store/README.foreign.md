...
> **DEMO CONSUMER.** Этот каталог содержит автономный Django 5 / PostgreSQL 16
> storefront, который используется как реальный demo tenant Helperium.
> Он живёт внутри `demo/`, но его runtime-код и deployment-конфигурация могут
> изменяться в этом репозитории, когда это необходимо для security, корректного
> дизайна интеграции или воспроизводимости демо.
...
1. Security fixes, integration fixes и исправления воспроизводимости в этом
   каталоге разрешены и должны сопровождаться targeted verification.
2. Изменения schema/миграций и seed-контракта должны оставаться совместимыми с
   `db/schema.sql` и tenant fixtures Helperium; при изменении контракта добавляй
   соответствующую regression/E2E-проверку.
3. **Изоляция:** код Helperium **не импортирует** модули из
   `demo/autoparts-store/`. Это автономное приложение, подключённое к Helperium
   через PostgreSQL и HTTP demo integration.
4. Не используй storefront как обход tenant scope, auth или read-only политики;
   credentials для Helperium и demo DB должны оставаться отдельными.
5. `db/schema.sql` и этот README считаются частью поддерживаемого demo-контракта;
   при изменении моделей проверяй их согласованность.
