...
- **fix(api):** enforce the retry deadline even when a transport ignores its timeout argument (hung DNS/TLS): CompletionRetryExecutor now wraps every physical attempt in asyncio.wait_for on the same per-attempt budget and classifies the resulting asyncio.TimeoutError as transient, so a hung attempt is cancelled at the deadline instead of blocking the agent loop forever. **Verification:** red-first regression `test_hung_attempt_is_cancelled_at_the_deadline`; full api suite 563 passed, ruff/pyright clean.
- **feat(api):** opt-in API_ENABLE_DOCS flag brings /docs, /redoc and /openapi.json back for local debugging only: FastAPI routes stay off by default (control-plane hardening, bd508c1), the flag is documented in .env.example and the api-service env table, api-contracts.md records the default-off decision and the unauthenticated-when-enabled caveat, and specs/README.md gains a programmatic schema-generation command that works without a running server. **Verification:** new `test_api_docs_flag.py` (default 404, falsy values stay off, flag serves docs unauthenticated); full api suite 563 passed; `make ci-docs` green.
...
- **feat(helperium-go):** single-source the repo-wide tenant-ID contract in config.ValidTenantID and migrate mcp-gateway scope validation, data-service admin add/update handlers and the admin-dashboard SQLite upload handler to it; crafted tenant IDs (path separators, dots, overlong) now fail closed with 400 before reaching filesystem paths or stores. **Verification:** per-service build+test+vet green with table-driven regressions; isolated Docker E2E 148 passed.
- **fix(admin-dashboard):** harden tenant SQLite upload inputs: validate the externally controlled tenant_id via the shared contract before any filepath math, whitelist .db/.sqlite/.sqlite3 extensions instead of trusting filepath.Ext(header.Filename), and stop test constructors from writing the audit log into the tracked internal/audit/ fixture dir (empty DataDir now resolves to a temp dir). **Verification:** admin-dashboard suite green with tenant_upload_validation_test.go regressions; isolated Docker E2E 148 passed.
- **test(e2e):** close the readonly sabotage gap with a mutation-tested probe: before/after table digests plus rejected write attempts assert guard reasons, and an executed sabotage drill (isValidFilterExpression always-true) fails all 7 probe assertions while the legacy suite stays green; lifecycle delete tests moved to throwaway tenants, tenant pickup polling replaces the blind sleep, healthcheck resolves isolated native ports. **Verification:** native E2E 148 passed twice, Docker E2E 148 passed.
- **docs:** archive the audit followup evidence (2026-08-31, f094429) and the widget demo readiness run (2026-09-01: HELPERIUM_AGENT/Agent Store name drift, missing native autoparts tenant registration causing MCP 500 + zero tools, agent loop limits; provider matrix with live fallback proof), refresh testing-guide and PENTEST-CHEK markers, CI E2E job gets a 20m timeout and full five-service log dump on failure, compose.sh test profile auto-rebuilds service images on up/run. **Verification:** check_docs_paths green (59 docs / 263 paths); make ci exit 0 on the final tree.

## 2026-08-30

- **fix(api):** keep tool schemas on continuation for deepseek models served through OpenAI-compatible relays: litellm.supports_function_calling false-negatives for relayed models, so the completion policy stripped tool schemas after the first tool_result and deepseek-v4-flash emitted its native tool-call markup as visible final text, breaking every multi-hop tool cycle on direct chat. **Verification:** live SSE smoke completed db_map -> db_get -> grounded final (real price from tenant DB) after the fix; API suite 483 passed with a test-only ENCRYPTION_KEY, pyright clean, policy regression added.
- **fix(ci):** repair the native E2E launcher contract: the e2e runner now mirrors the secure-profile variables (MCP_DEV=false, MCP_REQUIRE_AUTH=true, matching MCP keys, rate-limit floors) onto the pytest caller like compose.sh --profile test and the CI override do, and the web service receives DATA_SERVICE_URL pointing at the profile data port so /api/manifest proxying stops returning 500 on isolated ports. **Verification:** native E2E 138 passed after e2e-up (2 pre-existing launcher failures before the fix).
...
- **docs:** артефакт doc/archive/2026-08-07-demo-integration-audit.md (удалён) — 6 корней демо-проблем + 2 открытых фронтовых бага (app.js вкладки, admin-dashboard Alpine); карта §5b + метка верификации обновлены.
