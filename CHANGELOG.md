...
## 2026-08-18
- **fix(security/api-mcp):** Docker Compose больше не задаёт wildcard `CORS_ALLOW_ORIGINS=*` при отсутствующем deployment override: API fail-closed разрешает только localhost fallback, пока public embed origins не настроены явно. MCP gateway валидирует header-controlled tenant IDs до cache/manifest routing, поэтому traversal-style scope больше не может вызвать upstream lookup и public `500`. Добавлены runtime Docker E2E CORS regression, malformed tenant-scope coverage и повторно используемые локальные API/MCP barrier probes. **Проверка:** targeted Go/Python tests, clean Docker E2E **137 passed**, `make ci`.

- **fix(resilience/backend):** tenant registration теперь возвращает безопасный retryable `503 tenant_database_unavailable` без DSN/credentials при недоступной client DB. MCP gateway нормализует transport/5xx data-service outage в стабильный public message. API MCP client ограничивает зависший Streamable HTTP tool call жёстким deadline, превращает transport cancellation (включая retry path) в retryable ToolResult и изолирует agent producer от SSE writer: активная SSE-сессия после gateway outage получает sanitised `tool_result`, terminal `error` и `done`, а не обрывается после `tool_call`. Scripted-only post-tool-call gate добавлен для детерминированного fault injection. **Проверка:** Go/Python focused regressions, Docker tenant-DB unavailable E2E и manual active-SSE gateway-loss regression.
